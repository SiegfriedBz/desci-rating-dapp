...
**Why a durable job.** GROBID + Gemini + DKG publish routinely exceeds a single HTTP request budget — an observed end-to-end run took 6m 5s — so the web path enqueues an Inngest job (`finish: "10m"`, 2 retries) and the modal polls run status every 3 s.
