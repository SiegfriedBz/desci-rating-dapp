...
`dkg-publish` names the asset `desci-pub-<event id>`. The daemon returns the UAL of an already-published name — published meaning **minted**, not merely stored — instead of minting again, so a retry that follows a lost response reuses the first asset rather than duplicating it. A name whose RDF was stored before the retry killed the step is not reused: the retry finishes its mint. A new submission is a new event, so a deliberate republish still mints a fresh UAL — which `queryPublicationsWithRatings` depends on.
